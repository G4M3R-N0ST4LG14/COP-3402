// George Morillo & Arturo Lara
// COP 3402: Systems Software
// Homework 2: Lexical Analyzer (Optimized with Output File Generation)

// Libraries
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <ctype.h>

// Constants
#define MAX_IDENT_LEN 11
#define MAX_NUM_LEN 5
#define MAX_LEXEMES 1000

// Struct for assigning numerical values to each token
typedef enum 
{
    skipsym = 1, identsym, numbersym, plussym, minussym, multsym,
    slashsym, fisym, eqlsym, neqsym, lessym, leqsym,
    gtrsym, geqsym, lparentsym, rparentsym, commasym,
    semicolonsym, periodsym, becomessym, beginsym,
    endsym, ifsym, thensym, whilesym, dosym, callsym,
    constsym, varsym, procsym, writesym, readsym, elsesym
} TokenType;

// Struct to identify a token
typedef struct 
{
    TokenType type;
    char lexeme[MAX_IDENT_LEN + 1];
} Token;

// Global variables
Token tokens[MAX_LEXEMES];
int token_count = 0;
int error_found = 0;

// Function to store reserved words
char* reserved_words[] = 
{
     "const", "var", "procedure", "call", "begin", "end", "if", "fi","then", "else", "while", "do", "read", "write"
};

// Function to store reserved tokens
TokenType reserved_tokens[] = 
{
    constsym, varsym, procsym, callsym, beginsym, endsym,
    ifsym, fisym, thensym, elsesym, whilesym, dosym, readsym, writesym
};

// Function to add a token
void add_token(TokenType type, const char* lexeme) {
    if (token_count >= MAX_LEXEMES) {
        exit(1);
    }
    tokens[token_count].type = type;
    strncpy(tokens[token_count].lexeme, lexeme, MAX_IDENT_LEN);
    token_count++;
}

// Function to flag reserved words
int is_reserved(const char* word) {
    for (int i = 0; i < sizeof(reserved_words) / sizeof(char*); i++) {
        if (strcmp(reserved_words[i], word) == 0)
            return reserved_tokens[i];
    }
    return 0;
}

// Function to scan input and flag its identity
void scan(const char* filename) {
    FILE* file = fopen(filename, "r");
    if (!file) {
        printf("Cannot open file.\n");
        return;
    }

    char c;
    while ((c = fgetc(file)) != EOF) {

        if (isspace(c))
            continue;

        else if (isalpha(c)) {
            char buffer[MAX_IDENT_LEN + 1] = {0};
            int i = 0;
            buffer[i++] = c;

            while ((c = fgetc(file)) != EOF && isalnum(c)) {
                if (i < MAX_IDENT_LEN)
                    buffer[i++] = c;
                else {
                    buffer[i] = '\0';
                    while ((c = fgetc(file)) != EOF && isalnum(c));
                    printf("%s\t\tError: Name too long\n", buffer);
                    error_found = 1;
                    continue;  
                }
            }

            buffer[i] = '\0';
            if (c != EOF) ungetc(c, file);
            int res = is_reserved(buffer);
            if (res)
                add_token(res, buffer);
            else
                add_token(identsym, buffer);
            continue; 

        } else if (isdigit(c)) {
            char buffer[MAX_NUM_LEN + 1] = {0};
            int i = 0;
            buffer[i++] = c;

            while ((c = fgetc(file)) != EOF && isdigit(c)) {
                if (i < MAX_NUM_LEN)
                    buffer[i++] = c;
                else {
                    buffer[i] = '\0';
                    while ((c = fgetc(file)) != EOF && isdigit(c));
                    printf("%s\t\tError: Number too long\n", buffer);
                    error_found = 1;
                    continue;  
                }
            }

            buffer[i] = '\0';
            if (c != EOF) ungetc(c, file);
            add_token(numbersym, buffer);
            continue; 

        } else if (c == '/' && (c = fgetc(file)) == '*') {
            while ((c = fgetc(file)) != EOF) {
                if (c == '*' && (c = fgetc(file)) == '/')
                    break;
            }
            continue;

        } else {
            char sym[3] = {c, '\0', '\0'};
            if (c == ':' && (c = fgetc(file)) == '=') {
                sym[1] = '=';
                add_token(becomessym, sym);
                continue;
            } else {
                switch (sym[0]) {
                    case '+': add_token(plussym, sym); break;
                    case '-': add_token(minussym, sym); break;
                    case '*': add_token(multsym, sym); break;
                    case '/': add_token(slashsym, sym); break;
                    case '=': add_token(eqlsym, sym); break;
                    case '<': add_token(lessym, sym); break;
                    case '>': add_token(gtrsym, sym); break;
                    case '(': add_token(lparentsym, sym); break;
                    case ')': add_token(rparentsym, sym); break;
                    case ',': add_token(commasym, sym); break;
                    case ';': add_token(semicolonsym, sym); break;
                    case '.': add_token(periodsym, sym); break;
                    default:
                        printf("%s\t\tError: Invalid symbol\n", sym);
                        error_found = 1;
                        break;
                }
            }
        }
    }
    fclose(file);
}

void write_output(const char* input_filename, const char* output_filename) {
    FILE* in = fopen(input_filename, "r");
    FILE* out = fopen(output_filename, "w");
    if (!in || !out) {
        printf("Error opening files.\n");
        return;
    }

    fprintf(out, "Source Program:\n\n");
    char ch;
    while ((ch = fgetc(in)) != EOF)
        fputc(ch, out);
    fprintf(out, "\n");

    fprintf(out, "\nLexeme Table:\n\n");
    fprintf(out, "lexeme\t\ttoken type\n");
    for (int i = 0; i < token_count; i++)
        fprintf(out, "%-10s\t%d\n", tokens[i].lexeme, tokens[i].type);

    if (!error_found) {
        fprintf(out, "\nToken List:\n");
        for (int i = 0; i < token_count; i++) {
            fprintf(out, "%d ", tokens[i].type);
            if (tokens[i].type == identsym || tokens[i].type == numbersym)
                fprintf(out, "%s ", tokens[i].lexeme);
        }
        fprintf(out, "\n");
    }

    fclose(in);
    fclose(out);
}

int main(int argc, char* argv[]) {
    if (argc < 2) {
        printf("Usage: ./main.exe input.txt\n");
        return 1;
    }

    const char* output_filename = "output.txt";
    scan(argv[1]);
    write_output(argv[1], output_filename);
    return 0;
}
